...
const GraphicView = ({ data, headers }) => {
  ...
  const mappedData = useMemo(() => {
    if (data)
      return data.map((item, index) => {
        return {
          ...item,
          title: headers[classifierKey][index].title,
        };
      });
    return [];
  }, [data]);
  ...
  const slicedData = mappedData.slice(start, end);
  const slicedHeader = headers[classifierKey].slice(start, end);
  ...
  return (
    <>
      <div className='overflow-y-auto overflow-x-hidden max-h-[85%]'>
        <ResponsiveContainer width='100%' height={slicedData.length * 33 * l}>
          <BarChart barSize={25} layout='vertical' data={slicedData} barCategoryGap={10}>
            ...
            <YAxis
              ...
              textAnchor='end'
              onClick={(item) => {
                const { id, category_id } = slicedHeader[item.index];
