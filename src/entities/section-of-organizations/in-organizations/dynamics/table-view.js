...
const TableView = ({ data, headers }) => {
  ...
  const { title, executor_authority_id, has_child, type_title, type_id, is_additional, ...rest } =
    authority_ids.length || userDetails?.authority_id !== 1 ? data[0].children[0] : data[0] || {};
...
const BodyForOnlyOrganizations = ({ data, sortStates }) => {
  ...
  return (
    <Table.Tbody>
      {sortedData?.map(({ title, executor_authority_id, has_child, ...rest }, index) => {
        return (
...
const BodyForOrganizationsAndChildren = ({ data, sortStates }) => {
  return (
    <Table.Tbody>
      {data.map(({ title, data, children }, index) => {
        const { is_additional, ...rest } = data;
        return (
          <Fragment key={index}>
            <Table.Tr className='bg-slate-400 dark:bg-main_dark_blue'>
              <Table.Td align='center'>{index + 1}</Table.Td>
              <Table.Td>{title}</Table.Td>
              {Object.values(rest).map((count, index) => (
                <Table.Td align='center' key={index}>
                  {count}
                </Table.Td>
              ))}
            </Table.Tr>
...
const OrgChildren = ({ data, sortStates, parentIndex }) => {
  ...
  return sortedData?.map(
    (
      { executor_authority_id, title, has_child, type_title, type_id, is_additional, ...rest },
      childIndex,
    ) => (
      <Table.Tr key={childIndex}>
