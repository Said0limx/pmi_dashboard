  ...
  return (
    <div>
      <Modal
        ...
        title={t(`${initialValues ? 'Update' : 'Create'} scales setting`)}
        size={'xl'}
      >
...
const Form = ({ initialValues, close, onSuccess: OnS }) => {
  ...
  const onSuccess = () => {
    close();
    queryClient.invalidateQueries({ queryKey: ['admin/scales-setting/list'] });
    toast.success(t('Scale setting created'));
  };
  ...
  const createStrategy = (values) => {
    mutate(
      {
        url: initialValues
          ? `/admin/scales-setting/update/${initialValues.id}`
          : '/admin/scales-setting/create',
        data: values,
